...
// Using toISOString() shifted dates back one day for India (UTC+5:30).
...
export default function FitnessBusinessManager() {
  ...
  // NEW: Different reorder days for Gold (20) and Non-Gold (30)
  ...
  const getReorderDueDate = (purchaseDate, productQuantity) => {
    ...
    return toLocalDateString(purchase);
  };
  ...
  const handleWelcomeCallDone = (id) => {
    const updated = customers.map(c => c.id === id ? { ...c, welcomeCallDone: true, welcomeCallDate: toLocalDateString(new Date()) } : c);
    setCustomers(updated);
    saveCustomers(updated);
  ...
  const handleUpdatePurchaseDate = (id) => {
    ...
    if (window.confirm(`Mark new purchase today? Reorder reminder will be set for ${reorderDays} days from now (${isGoldCustomer(customer.productQuantity) ? 'Gold customer' : 'Non-Gold customer'}).`)) {
  ...
  const exportToExcel = () => {
    ...
    const fileName = `Fitness_Customers_${toLocalDateString(new Date())}.xlsx`;
              ...
              {alerts.reorderDueGold.map(c => (
                <div key={`rg-${c.id}`} style={styles.taskItem}>
                  <label style={styles.taskLabel}>
                    ...
                    <strong>{c.name}</strong> ({c.daysSincePurchase}d since purchase, 20d cycle)
                  </label>
                </div>
              ))}
              {alerts.reorderDueNonGold.map(c => (
                <div key={`rn-${c.id}`} style={styles.taskItem}>
                  <label style={styles.taskLabel}>
                    ...
                    <strong>{c.name}</strong> ({c.daysSincePurchase}d since purchase, 30d cycle)
                  </label>
                </div>
              ))}
            </>
                    ...
                    {formData.productQuantity && (
                      <p style={styles.helperText}>
                        {isGoldCustomer(formData.productQuantity) 
                          ? '👑 GOLD - Reorder reminder every 20 days' 
                          : '👤 Non-Gold - Reorder reminder every 30 days'}
                      </p>
                    )}
                  </div>
                  <div style={styles.formGroup}>
                  ...
                  return (
                    <div key={customer.id} style={{...styles.customerCard, ...(isGold ? styles.goldCard : {}), ...(noResponseAlert ? styles.urgentCard : {})}}>
                      ...
                      <div style={styles.cardContent}>
                        <p><strong>📅 Joining:</strong> {customer.dateOfJoining}</p>
                          ...
                          <p><strong>🛒 Last Purchase:</strong> {customer.lastPurchaseDate}
                            {daysUntilReorder !== null && (
                              <span style={{
                                ...
                                {daysUntilReorder <= 0 ? `⚠️ REORDER NOW! (${reorderDays}d cycle)` : `(${daysUntilReorder} days, ${reorderDays}d cycle)`}
                              </span>
                            )}
                          </p>
                        )}
                        {customer.lastResponseDate && (
                          <p><strong>💬 Last Response:</strong> {customer.lastResponseDate}
                            <span style={{ marginLeft: '10px', color: noResponseAlert ? '#c0392b' : '#27ae60', fontWeight: 'bold' }}>
                              ({daysSinceResponse} days ago)
                            </span>
                          </p>
                        )}
                      ...
                      <div style={styles.cardStatus}>
                        <div style={{...styles.statusItem, borderLeft: customer.welcomeCallDone ? '4px solid #27ae60' : '4px solid #e74c3c'}}>
                          <strong>Welcome Call:</strong>
                          <span style={{color: customer.welcomeCallDone ? '#27ae60' : '#e74c3c', marginLeft: '10px'}}>
                            {customer.welcomeCallDone ? `✅ ${customer.welcomeCallDate}` : '❌ Pending'}
                          </span>
                        </div>
                      </div>
                ...
                <div style={styles.alertGrid}>
                  {alerts.reorderDueGold.map(c => (
                    <div key={c.id} style={{...styles.alertCard, borderLeft: '5px solid #f1c40f'}}>
                      ...
                      <p><strong>Days Since:</strong> {c.daysSincePurchase} (Gold cycle: 20 days)</p>
                ...
                <div style={styles.alertGrid}>
                  {alerts.reorderDueNonGold.map(c => (
                    <div key={c.id} style={styles.alertCard}>
                      <h4>{c.name}</h4>
                      <p><strong>Last Purchase:</strong> {c.lastPurchaseDate}</p>
                      <p><strong>Days Since:</strong> {c.daysSincePurchase} (Non-Gold cycle: 30 days)</p>
                      <button onClick={() => handleUpdatePurchaseDate(c.id)} style={{...styles.btnSuccess, width: '100%'}}>🛒 Mark New Purchase</button>
                    </div>
                  ))}
                </div>
                ...
                <div style={styles.alertGrid}>
                  {alerts.welcomeCallPending.map(c => (
                    <div key={c.id} style={styles.alertCard}>
                      <h4>{c.name}</h4>
                      <p><strong>Joined:</strong> {c.dateOfJoining}</p>
                      <p><strong>Days:</strong> {calculateDaysSinceJoining(c.dateOfJoining)}</p>
...
const styles = {
  ...
  excelBtn: { padding: '12px 20px', backgroundColor: '#27ae60', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' },
  tasksBtn: { padding: '12px 20px', backgroundColor: 'white', color: '#667eea', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' },
